...
RSpec.describe Odin::Transform::TransformEngine do
  ...
  describe "boolean operators in :if conditions" do
    def run(condition, source)
      text = <<~ODIN
        {$}
        odin = "1.0.0"
        transform = "1.0.0"
        ...
        Hit = "yes"
      ODIN
      execute_transform(text, source)
    end

    it "includes when AND is true" do
      result = run("@.a = true and @.b = true", { "a" => true, "b" => true })
      expect(result.output["Out"]["Hit"]).to eq("yes")
    end

    it "omits when AND has a false factor" do
      result = run("@.a = true and @.b = true", { "a" => true, "b" => false })
      expect(result.output).not_to have_key("Out")
    end

    it "includes when OR has a true term" do
      result = run("@.a = true or @.b = true", { "a" => false, "b" => true })
      expect(result.output["Out"]["Hit"]).to eq("yes")
    end

    it "omits when OR has all false terms" do
      result = run("@.a = true or @.b = true", { "a" => false, "b" => false })
      expect(result.output).not_to have_key("Out")
    end

    it "negates a truthy path with NOT" do
      result = run("not @.flag", { "flag" => false })
      expect(result.output["Out"]["Hit"]).to eq("yes")
    end

    it "negates a comparison with NOT" do
      result = run("not @.a = true", { "a" => false })
      expect(result.output["Out"]["Hit"]).to eq("yes")
    end

    it "binds NOT tighter than AND" do
      # not @.a = true  ->  false ; AND with true -> false
      result = run("not @.a = true and @.b = true", { "a" => true, "b" => true })
      expect(result.output).not_to have_key("Out")
    end

    it "binds AND tighter than OR" do
      # (false and false) or true -> true
      result = run("@.a = true and @.b = true or @.c = true",
                   { "a" => false, "b" => false, "c" => true })
      expect(result.output["Out"]["Hit"]).to eq("yes")
    end

    it "does not split operator words inside quoted values" do
      result = run("@.label = 'cats and dogs'", { "label" => "cats and dogs" })
      expect(result.output["Out"]["Hit"]).to eq("yes")
    end

    it "treats operators case-insensitively" do
      result = run("@.a = true AND @.b = true", { "a" => true, "b" => true })
      expect(result.output["Out"]["Hit"]).to eq("yes")
    end
  end
