  ...
  module Transform
    class TransformEngine
      ...
      def evaluate_condition(condition_str, source, context)
        ...
        evaluate_condition_or(condition_str.strip, source, context)
      end

      # OR layer — lowest precedence
      def evaluate_condition_or(expr, source, context)
        terms = split_top_level_condition(expr, "or")
        return terms.any? { |t| evaluate_condition_and(t, source, context) } if terms.length > 1

        evaluate_condition_and(expr, source, context)
      end

      # AND layer
      def evaluate_condition_and(expr, source, context)
        factors = split_top_level_condition(expr, "and")
        return factors.all? { |f| evaluate_condition_not(f, source, context) } if factors.length > 1

        evaluate_condition_not(expr, source, context)
      end

      # NOT layer — tightest precedence
      def evaluate_condition_not(expr, source, context)
        str = expr.strip
        if str.length >= 4 && str[0, 3].downcase == "not" && (str[3] == " " || str[3] == "\t")
          return !evaluate_condition_not(str[4..], source, context)
        end

        evaluate_condition_primary(str, source, context)
      end

      # Split on a top-level operator word, respecting quoted strings.
      def split_top_level_condition(expr, op)
        segments = []
        start = 0
        i = 0
        in_single = false
        ...
        while i < expr.length
          ...
          if in_single
            ...
            in_double = true
          elsif i + op_len <= expr.length && expr[i, op_len].downcase == op &&
                i > 0 && whitespace?(expr[i - 1]) &&
                i + op_len < expr.length && whitespace?(expr[i + op_len])
            segments << expr[start...i].strip
            i += op_len
            start = i
            next
          end
      ...
      def evaluate_condition_primary(condition_str, source, context)
